...
class Ando_Regex {
     ...
     * excluding deprecated ones. At the moment, only 'e' is deprecated.
    ...
    static public function global_safe_modifiers() {
        $result = ''  // i m s x A D S U X u
            . self::PCRE_CASELESS_MODIFIER
            . self::PCRE_MULTILINE_MODIFIER
            . self::PCRE_DOTALL_MODIFIER
            . self::PCRE_EXTENDED_MODIFIER
            . self::PCRE_ANCHORED_MODIFIER
            . self::PCRE_DOLLAR_ENDONLY_MODIFIER
            . self::PCRE_STUDY_EXTRA_NEEDED_MODIFIER
            . self::PCRE_UNGREEDY_MODIFIER
            . self::PCRE_EXTRA_MODIFIER
    ...
    static public function pattern_quoted_string( $begin = "'", $end = "'", $escape = "\\\\" ) {
        ...
        $regex->interpolate( array( 'begin' => $begin, 'end' => $end, 'escape' => $escape ) );
    ...
    public function template_set( $template ) {
        if ( ! self::is_valid_template( $template ) ) {
            throw new Ando_Regex_Exception( 'Invalid template.' );
        }
    ...
    public function wrapper_set( $wrapper ) {
        if ( ! self::is_valid_wrapper( $wrapper ) ) {
            throw new Ando_Regex_Exception( 'Invalid wrapper.' );
        }
    ...
    static public function unwrap( $expression, &$wrapper = null ) {
        ...
        if ( preg_match( '@' . self::option_same_name_groups_ok() . '^(?:' . $delimited . ')'
            . '(?<modifiers>' . self::pattern_global_modifiers() . ')$@', $no_esc, $matches ) ) {
